...
function SubtractAmount(item){
  const num=(cart.value.filter(cartItem => cartItem === item).length);
        ...
        <div class="flex justify-center ">
          <div class="container flex justify-center flex-wrap bg-neutral-content rounded-xl w-72">
            <Cart v-for="item in cart"
              ...
              :img="item.img"
              :price="item.price"
            ><button @click="SubtractAmount(item)" class="mb-5 pt-2 pb-1 bg-base-100 hover:bg-neutral-content hover:text-neutral hover:text-base duration-500 rounded-2xl w-1/2 text-sm">Remove {{ item.name }}</button>
          </Cart>
          </div>
      </div>
      </div>
